...
def subsample_and_propagate_labels(
    ...
    from .preprocess import prepare_mixed_arrays_no_label
...
def stratified_landmarks(
    ...
    """
    Fallback landmark selection: stratified random sampling from full labels.
    Guarantees at least per_cluster_min landmarks per cluster (if cluster is large enough),
    distributes remaining quota proportionally to cluster size.

    Parameters
    ----------
    labels_full : np.ndarray, shape (n,)
        Cluster label for every point in the full dataset.
    m_cap : int
        Total number of landmarks to select.
    per_cluster_min : int
    ...
    -------
    np.ndarray
        Sorted array of selected landmark indices (global row indices).
    """
    ...
    # Step 1: assign minimum quota per cluster (capped by cluster size)
    quota = {}
    for c, cnt in zip(uniq, counts):
        quota[c] = min(per_cluster_min, int(cnt))

    allocated = sum(quota.values())
    remaining = max(0, m_cap - allocated)

    # Step 2: distribute remaining proportionally to cluster size
    if remaining > 0:
        total_n = n
        for c, cnt in zip(uniq, counts):
            extra = int(round(remaining * (cnt / total_n)))
            quota[c] += extra

    # Step 3: re-cap each cluster at its actual size, adjust total
    for c, cnt in zip(uniq, counts):
        quota[c] = min(quota[c], int(cnt))

    # Step 4: sample from each cluster
    selected = []
    for c in uniq:
        idx_c = np.where(labels_full == c)[0]
    ...
    if not selected:
        ...
        return np.sort(rng.choice(n, size=min(m_cap, n), replace=False))

    landmarks = np.unique(np.concatenate(selected))

    # If still short of m_cap, fill with random non-selected points
    if len(landmarks) < m_cap:
        rest = np.setdiff1d(np.arange(n), landmarks)
        if len(rest) > 0:
            fill = rng.choice(rest, size=min(m_cap - len(landmarks), len(rest)), replace=False)
            landmarks = np.sort(np.concatenate([landmarks, fill]))

    return landmarks[:m_cap]
